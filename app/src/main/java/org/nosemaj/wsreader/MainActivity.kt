...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupWebSocket() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            client.wss(HttpMethod.Get, url.host, url.port, url.encodedPath) {
                addStatus(R.string.websocket_connected, url.toString())
                awaitAll(async {
                    commands
                        ...
                        .collect { json -> outgoing.send(Frame.Text(json)) }
                }, async {
    ...
    private fun addStatus(@StringRes resId: Int, arg: String) {
        val line = getString(R.string.status_line, lineNumber, getString(resId, arg))
